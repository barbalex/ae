...
export default React.createClass({
  ...
  render () {
    ...
    const tooltip = <Tooltip id={id}>Gruppen: {fNameObject.groups.join(', ')}<br/>Anzahl Objekte: {fNameObject.count}<br/>Feldtyp: {fNameObject.fType}</Tooltip>
    ...
    return (
      <OverlayTrigger placement='left' trigger={['click']} rootClose overlay={tooltip}>
        <Button bsSize='small' style={buttonStyle}><Glyphicon glyph='info-sign' style={{fontSize: 14}} /></Button>
      </OverlayTrigger>
    )
  }
})
