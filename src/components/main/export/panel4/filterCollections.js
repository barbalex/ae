  ...
  Object.keys(exportOptions).forEach((cName) => {
    ...
    if (cType) {
      Object.keys(exportOptions[cName]).forEach((fName) => {
         ...
         * if taxonomienZusammenfassen, exclude cType === 'taxonomy'
         */
        if (cName !== 'object' && fName !== 'cType' && !(cType === 'taxonomy' && taxonomienZusammenfassen)) {
          ...
          objects = _.filter(objects, (object) => {
            ...
            let collections = []
            if (cType === 'taxonomy' && !taxonomienZusammenfassen) collections = object.Taxonomien
