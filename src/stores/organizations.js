...
export default (Actions) => Reflux.createStore({
  ...
  onGetOrganizations(email) {
    ...
    app.remoteDb.query('organizations', { include_docs: true })
      .then((result) => {
        ...
        const orgsWhereUserIsAdmin = organizations.filter((org) =>
          org.orgAdmins.includes(email)
        )
        this.userIsAdminInOrgs = _map(orgsWhereUserIsAdmin, 'Name')
        ...
        let orgsWhereUserIsEsWriter = organizations.filter((org) =>
          org.esWriters.includes(email)
        )
        orgsWhereUserIsEsWriter = union(orgsWhereUserIsEsWriter, orgsWhereUserIsAdmin)
        this.userIsEsWriterInOrgs = _map(orgsWhereUserIsEsWriter, 'Name')
        // is user lr-writer in orgs?
        let orgsWhereUserLrEsWriter = organizations.filter((org) =>
          org.lrWriters.includes(email)
        )
  ...
  onRemoveUserFromActiveOrganization(user, userFieldName) {
    ...
    if (activeOrganization[userFieldName]) {
      ...
      removeRolesFromUser(user, roles)
        .then(() => {
          activeOrganization[userFieldName] = activeOrganization[userFieldName].filter((esW) =>
            esW !== user
          )
  ...
  onGetTcsOfOrganization(orgName) {
    app.taxonomyCollectionsStore.getTcs()
      .then((tcs) => {
        this.tcsOfActiveOrganization = tcs.filter((tc) =>
          tc.organization === orgName
        )
        this.triggerMe()
      })
      .catch((error) =>
